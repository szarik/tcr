        ...
        var DRPTemplate = '<div class="daterangepicker dropdown-menu">' +
                ...
                '<div class="ranges">' +
                  '<div class="range_inputs">' +
                    ...
                    '</div>' +
                    '<button class="btn btn-small btn-success" disabled="disabled" onclick="submitDates()">' + this.locale.applyLabel + '</button>' +
                    '<br/><br/><br/>' +
                    '<br/><button style="width: 70px" class="btn btn-small btn-info" disabled="disabled" onclick="setDate(new Date())">Dziś</button>' +
                    ...
                    '<br/><button style="width: 70px" class="btn btn-small btn-danger" disabled="disabled" onclick="setDate(new Date(new Date().getTime()+2*(24*60*60*1000)))">Pojutrze</button>' +
                  '</div>' +
                '</div>' +
              '</div>';
...
//        this.container.find('.calendar').on('mouseenter', 'td.available', $.proxy(this.enterDate, this));
        ...
        // TCR custom
        this.container.find('.ranges').on('click', 'button', $.proxy(this.updateDates, this));
...
//            this.hide();
        ...
        updateDates: function () {
        	this.endDate = new Date($('input[name=daterangepicker_end]').val());
        	this.updateCalendars();
        	this.startDate = new Date($('input[name=daterangepicker_start]').val());
        	this.updateCalendars();
        },
...
function setDate(date) {
	...
	$('input[name=daterangepicker_start]').val(value);
	$('input[name=daterangepicker_end]').val(value);
}

function submitDates() {
	var date1 = $('input[name=daterangepicker_start]').val();
	var date2 = $('input[name=daterangepicker_end]').val();
	
	if(date1 == date2) {
		ustawFiltr('data', date1, true);
	}
	else {
		ustawFiltr('data', date1+'_'+date2, true);
	}
}
